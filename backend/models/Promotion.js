...
const promotionSchema = new mongoose.Schema(
  {
    ...
    code: {
      type: String,
      unique: true,
      sparse: true, // null nếu tự động áp dụng
      uppercase: true,
      trim: true,
      default: null,
    },
...
promotionSchema.index({ isAutoApply: 1, status: 1 });
...
promotionSchema.methods.isApplicableToProduct = function (
  productId,
  categoryId,
) {
  ...
  switch (this.applyTo) {
    case APPLY_TO.ALL:
      return true;
    case APPLY_TO.PRODUCTS:
      return this.applicableProducts.map(String).includes(pid);
    case APPLY_TO.CATEGORIES:
      return !!cid && this.applicableCategories.map(String).includes(cid);
    default:
      return false;
  }
};
...
promotionSchema.methods.incrementFlashSaleSold = async function (
  productId,
  quantity = 1,
) {
  await this.constructor.updateOne(
